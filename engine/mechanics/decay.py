...
class InfrastructureManager:
    ...
    def _cfg(engine) -> dict:
        ...
        return cfg_get(engine.config, "infraestrutura")
    ...
    @staticmethod
    def _aplicar_consequencias(engine, local: Local, local_id: str, cfg: dict):
        ...
        if estado == EstadoInfraestrutura.RUINA and local.tipo != TipoLocal.RUINA.value:
            InfrastructureManager._colapsar_para_ruina(engine, local, local_id)

        elif estado == EstadoInfraestrutura.CRITICO and local.status == 1:
            InfrastructureManager._fechar_local(engine, local, local_id)
    ...
    @staticmethod
    def _fechar_local(engine, local: Local, local_id: str):
        """
    ...
    @staticmethod
    def _colapsar_para_ruina(engine, local: Local, local_id: str):
        """
    ...
    @staticmethod
    def processar_reparos_espontaneos(engine):
        """
        ...
        locais_danificados = [
            (l_id, l) for l_id, l in engine.locais.items()
