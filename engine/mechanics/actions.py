...
class NPCActionManager:
    ...
    def _executar_comer(engine, npc: NPC, cfg_acoes: dict, cfg_bio: dict):
        ...
        if is_dependent:
            ...
            num_dependentes = NPCUtils.contar_dependentes_na_casa(engine.npcs, npc)
        ...
        if pagador.dinheiro_total_pc >= custo_do_tick:
            ...
            if not sopao_ok and WorldLogger.deve_logar_amostra(engine.tick_count, engine.config):
                if is_dependent:
    ...
    def _executar_construir(engine, npc: NPC):
        ...
        if obra.integridade >= 100:
            ...
            dono = next((n for n in engine.npcs if n.id == obra.dono_npc_id), None) or npc
