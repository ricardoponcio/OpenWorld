...
| T02 | 2026-09-15 | 30 min reais, mundo repovoado de E01 (30.936 habitantes), `run_simulation.py` + `run_dashboard.py` reais (não cópia), velocidade pedida 21600×, "painel aberto" simulado com requisições HTTP de verdade (sem browser) alternando Habitantes/Estatísticas/Mapa Live a cada 3s + `/api/estado` a cada 1s. **0 erros HTTP em 1.939 chamadas, 0 Traceback no terminal da simulação.** `/api/estado`: 1.744 chamadas, **máx 3.248 B** (<5KB ✅), tempo médio 31,3ms/p95 37,3ms (<50ms ✅ — um outlier de 4,5s isolado, não repetido). Habitantes: 195 chamadas, médio 42,2ms (um outlier de 1,9s). Dia 1 06:02 → Dia 8 12:50 (**~7,3 dias simulados**). Velocidade efetiva na leitura final: **224×** — bem abaixo dos 631-890× da Parada 1/2 (ver divergência). `ms_por_tick`: médio 338,9ms / p95 1.172,9ms (também bem acima da Parada 1). `world.log` +14,4 MB em ~33 min real (dentro do teto de 20MB/10min da Parada 1, pro-rata). WAL: 9,15 MB ao fim (<128 MB ✅), checkpoint ativo. Estatísticas "hoje" (insumo pra decisão de economia pendente): 206 nascimentos, 245 óbito_saúde + 264 óbito_velhice = **509 óbitos**, 160.930 minuto_em_inanição (contador, não pessoas), 5 casamentos; população viva caiu de 30.936 para **26.868** (-4.068) em ~7 dias — consistente com o número já citado na Seção de Decisões Pendentes (804 mortes por inanição em 3,5 dias). | **Achado real**: a velocidade efetiva (224× / ms-por-tick 339-1173ms) ficou bem abaixo da Parada 1 (631-890×) — a run desta tarefa é a PRIMEIRA vez que o laço real de `run_simulation.py` exercita o RoteadorIA/AIClient novo (Bloco I) sob carga de verdade: 206 nascimentos no período, cada um disparando uma thread que chama `nome_bebe` (só `ollama_local` na cadeia, sem OpenRouter — sem `OPENROUTER_API_KEY` no ambiente desta sessão). O log mostra repetidas "cadeia esgotada para nome_bebe" — caindo no fallback procedural (0 Traceback, nome de bebê sempre gerado, exatamente o comportamento desenhado). `curl` direto em `/v1/chat/completions` do Ollama respondeu em 2,3s (dentro do timeout de 8s) quando testado ISOLADO — a hipótese mais provável é fila/concorrência: muitas threads de batizado simultâneas disputando a MESMA instância local do Ollama (sem fila nem limite client-side, já que `ollama_local` tem `requisicoes_por_minuto=0`), cada uma esperando as anteriores terminarem até estourar o timeout de 8s. Não investigado a fundo nem corrigido (fora do escopo desta tarefa — T02 é validação, não otimização); registrado pra decisão do dono do projeto: aumentar `nome_bebe.timeout_s`, adicionar um limite client-side ao `ollama_local`, ou aceitar a degradação sob picos de natalidade. `run_simulation.py`/`run_dashboard.py` deixados RODANDO ao fim desta tarefa (é o estado estacionário normal do app, não um artefato de teste a limpar). |
