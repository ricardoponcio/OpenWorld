...
> [`12_PLANO_CIDADE_VIVA.md`](12_PLANO_CIDADE_VIVA.md), foi executado por inteiro (31 tarefas,
...
>
> 👉 **Leia o `12_PLANO_CIDADE_VIVA.md` antes deste.** Quase toda tarefa aqui mexe em código
...
>
> 👉 Leia [`11_ARQUITETURA.md`](11_ARQUITETURA.md) Seção 2 (regra de dependência entre camadas)
...
O *Registro de execução* do `12_PLANO_CIDADE_VIVA.md` deixou quatro perguntas em aberto.
...
`mundo_meta` — barato, respeita a "regra de processo" da `11_ARQUITETURA.md` (só
...
As decisões D1–D6 do `12_PLANO_CIDADE_VIVA.md` continuam valendo. Estas são novas, todas
...
projeto (`11_ARQUITETURA.md` Seção 7: as dependências são montadas uma vez por processo, no
...
5. ⚠️ **Não instancie um `SimulationEngine` no processo do Flask** para "resolver de vez".
   A `11_ARQUITETURA.md` (Seção 1, regra de processo) diz que só `run_simulation.py` é dono da
...
que é um NPC nem o que é uma família** (`11_ARQUITETURA.md` Seção 2). Se você acabar
...
O `12_PLANO_CIDADE_VIVA.md` tem o glossário de geometria urbana (banda, vão, faixa de
...
> [`14_PLANO_AVANCO_E_CALIBRAGEM.md`](14_PLANO_AVANCO_E_CALIBRAGEM.md).** Em resumo: o teto
...
| L02 | 2026-09-12 | `"sem_via"` documentado como valor válido de `classes_aresta`/`tipo_via` (base.py, ao lado de `Quadra`/`Rua`). `organica.py` trocou as duas atribuições ("aresta externa"/"aresta interna" no vão aberto) de `"servico"` pra `"sem_via"`. `lotes.py`: `_lotes_da_faixa` deixa de ser chamada pra aresta `"sem_via"` (nos dois ramos — com e sem pátio); `preparar_quadra` descarta a quadra com as 4 arestas `"sem_via"` (mesmo critério de "degenerada" já usado). `distancia_faixa_dominio` (base.py) e a cópia local em `gerador.py` devolvem 0.0 pra `"sem_via"` — não há via pra recuar. `grep -rn '"servico"' cartographer/` confirmou: as únicas ocorrências restantes são vielas de verdade (expansao.py, gerador.py, linear.py, o corte de `_cortar_quadra_funda`) — nenhuma precisou virar `"sem_via"`. `TOLERANCIA_SEM_FRENTE_PCT` em `audit_cidades.py` **não mudou** (continua 0.0, como o plano pede — "ela estava certa o tempo todo"). `tests/test_cidades.py::test_sem_lote_em_aresta_cega` (novo, W01) + o teste de coerência rua/quadra passou a pular `"sem_via"` também (por definição não tem rua próxima). **Resultado medido, 15 cidades reais regeradas: `sem_front% = 0.0` em TODAS — `audit_cidades.py` sai 0 pela primeira vez.** Uma quadra foi descartada em Cidade das Flores (80→79) e uma em Fenelburgo (210→209) por terem as 4 arestas cegas — não contei via log permanente (o plano pedia "contar quantas", medido aqui pela diferença de quadras no audit antes/depois, não instrumentado no runtime). Regeração determinística confirmada. Suíte: 142 passed. |
| L03 | 2026-09-12 | `GeradorCidade._gerar_quarteiroes_e_lotes` e `expansao.py::_gerar_features_e_lotes` passaram a derivar `rng_quadra = random.Random(self.seed ^ zlib.crc32(quarteirao_id_str.encode("utf-8")))` por quadra, em vez de passar o `rng`/`self.rng` sequencial e compartilhado — `self.seed` (GeradorCidade) e `seed_expansao` (expansao.py) já eram as sementes corretas do projeto, reusadas, nunca recalculadas. `self.rng`/`rng` (o compartilhado) continua existindo e sendo usado pra sorteios que são de fato por-cidade/por-arrabalde (ex.: `lote_fator_cidade`) — só a subdivisão em lotes de CADA quadra passou a ser independente. **Isto fecha o pendente de V01**: `tests/test_cidades.py::test_id_de_lote_estavel_quando_uma_quadra_e_removida` (novo) monta a malha, remove uma quadra do meio, gera de novo, e confirma que os ids de lote de TODAS as outras quadras (não só o formato, a contagem inteira) ficam idênticos — algo que só é verdade agora que cada quadra tem RNG próprio. Regeneradas as 15 cidades reais: `audit_cidades.py` continua saindo 0 (nenhuma regressão de bowtie/anel/frente); as contagens de lote mudaram um pouco em todas as cidades (esperado — armadilha 1, a ordem de consumo de RNG mudou), e a regeração continua determinística (`md5sum` idêntico em duas rodadas). Suíte: 143 passed. |
| L04 | 2026-09-12 | Duas colunas novas: `larg/prof` (mediana da razão entre as duas dimensões da quadra — pareia arestas OPOSTAS do quarteirão, 0-2 e 1-3, e trata o par mais curto como profundidade; verdadeiro pra radial/organica, aproximação razoável pra grade/linear) e `lotes/raio2` (a densidade que R02 vai calibrar por modelo). Invariante novo `4 <= mediana(lotes/quadra) <= 30`, violação dura como as demais. Cabeçalho do arquivo ganhou o aviso sobre `\| tail` engolir o código de saída. **Achado honesto, não escondido**: rodando contra as 15 cidades reais de hoje, `audit_cidades.py` volta a sair **1** — não por bug nenhum em L04, mas porque as medianas de `radial`/`organica` (30-41, medidas depois de S01/S02/S03) ainda passam do teto de 30 que este invariante novo passa a exigir. Investiguei a causa: mesmo uma quadra PERFEITAMENTE quadrada no alvo de S01 (`cidade_geo_quadra_largura_alvo_m = 95`, igual ao `vao_anel_alvo_m`) tem perímetro ~380m, que a ~11m de frente por lote já dá ~34 lotes — acima de 30 mesmo sem nenhum "erro de tolerância". Calibrar isso de verdade (achar o `largura_alvo` ou a relação frente/perímetro que fecha a conta em ≤30 para radial/organica, do jeito que grade/linear já fecham com quadras de 45-95m de lado) é exatamente o que R02 (`calibrar_densidade.py`, medição por modelo, não dedução) foi desenhado para fazer — e o próprio plano manda fazer isso DEPOIS de S e L inteiros (armadilha 8), não antes. Não toquei em `cidade_geo_quadra_largura_alvo_m` por conta própria: é uma decisão de calibração que pertence à R02, e mexer nela agora seria antecipar essa tarefa sem a medição que ela exige. **Decisão pendente pro dono do projeto**: usar R02 para fechar isso de verdade (provável), ou aceitar um teto de mediana maior que 30 especificamente para radial/organica. Suíte: 143 passed (o invariante novo do audit_cidades.py não faz parte da suíte pytest — é uma ferramenta de linha de comando à parte, como sempre foi). |
| R01 | 2026-09-14 | Destravada por C01/C02 (`14_PLANO_AVANCO_E_CALIBRAGEM.md`) — a dependência que tinha parado a sessão anterior (`cidade_geo_quadra_largura_alvo_m` mudando depois de R02) não se concretizou. `escala.py` ganhou `domicilios_alvo`/`lotes_alvo`/`raio_para_lotes`; `cidade_geo_raio_m_faixa_por_tamanho` virou só piso/teto de sanidade (`raio_para_lotes` grampeia nele, e loga quando grampeia — "se o grampo morder, registre, não engula"). `cidade_geo_escala` (D5) passou a multiplicar DOMICÍLIOS, não raio. Config nova `cidade_geo_domicilios_alvo_faixa_por_tamanho`, colocada em `cartografia` (não `geracao_urbana`, como o texto sugeria) — mesmo motivo de todo `cidade_geo_*` vizinho: `escala.py` recebe `CARTOGRAPHER_CONFIG`, não o config inteiro. Problema do ovo-e-a-galinha (`SitioCidade.medir()` precisa de um raio pra dimensionar a janela de terreno ANTES do modelo existir, que é quem sabe o raio real): resolvido usando o TETO de `faixa_raio_m` direto pra janela — sempre seguro, porque o raio final está sempre grampeado dentro dela. `derivar_ou_forcar_raio` extraída pra `base.py` (armadilha 12/DRY do próprio 11_ARQUITETURA.md) — evita triplicar a lógica em radial/grade/linear (organica herda de radial). `raio_m_forcado` (novo parâmetro opcional nos 4 modelos) existe só pra sustentar a calibração de R02 gerando cidades em raios controlados. Confirmado por teste ad-hoc que a construção falha alto (`KeyError`) antes de `cidade_geo_densidade_lote_por_modelo` existir (P5 — falhar alto é intencional aqui, não um bug). |
| R02 | 2026-09-14 | `builder/fix/calibrar_densidade.py` (novo): gera a mesma cidade-semente em dois raios (300m/900m), 5 sementes por ponto, mediana, ajusta `lotes = k·raio^e` por modelo. Medido: `radial` k=0,014157 e=1,9654; `grade` k=0,009881 e=1,9881; `organica` k=0,019949 e=1,8882; `linear` k=0,048614 e=1,5064 — os três "disco" (e≈2) e o "fita" (e≈1,5) bateram com a expectativa qualitativa do plano. Colado em `cidade_geo_densidade_lote_por_modelo`. **Ação 5 (correção de Newton) implementada depois, ao escrever o teste que faltava do W01** (ver linha W01 abaixo): `escala.corrigir_raio_por_newton` + `geometria/manifesto.py` (fio de produção — `calibrar_densidade.py` continua sem chamá-la, ela mede a curva crua de propósito). Achado que motivou a implementação: nos dois pontos calibrados (300m/900m) o ajuste bate quase exato (<1% de desvio), mas INTERPOLAR pra um raio no meio da faixa (não um dos dois pontos) desvia até 30-93% em `grade`/`linear` numa cidade só — a curva de potência de 2 pontos não captura toda a forma real. A correção de Newton (`raio_novo = raio_usado * (alvo/real)^(1/e)`, uma vez só, regrampeado) reduz drasticamente o pior caso (uma cidade `linear` a 742m, medida em R04, tinha 46% de desvio antes e 9,5% depois), mas não zera a variância cidade-a-cidade — terreno/ocupação por quadra continuam dando ruído real, sobretudo em `linear` (uma faixa "fita" é estruturalmente mais sensível ao sorteio de curvatura/afunilamento que um disco). É por isso que o teste W01 mede MEDIANA de várias sementes, não uma cidade só — igual a este próprio script. |
| R03 | 2026-09-14 | `RepositorioLote.contar_residencias_ocupadas(cidade_id)` (novo): `JOIN lotes`/`locais`, `estado='ocupado' AND categoria='residencia'` — a fonte de verdade no banco (T01), usada como checagem de consistência contra a lista de casas montada na importação da geometria (que é quem de fato guia a atribuição casa→família, já que carrega os ids). `builder/populador.py`: `_populacao_da_cidade`/`npcs_por_cidade_por_tamanho` saíram; `_familias_da_cidade` + `_gerar_dnas_em_paralelo` reescritos pra uma família por residência ocupada, tamanho sorteado em `npcs_por_familia_faixa` (nova, `geracao_populacao`, inicial `[2, 4]` — recalibrada em R04 pra `[2, 3]`). `npcs_por_cidade`/`npcs_por_cidade_por_tamanho` removidos de `config.json` e dos dois arquivos (`populador.py`, `populate.py`) — armadilha 12, duas fontes pra mesma quantidade. `--npcs` da CLI (`populate.py`) removido junto — não sobrou nenhuma "base" pra sobrescrever, população é 100% função de residências × família. `builder/reset_world.sh` atualizado (não chamava mais `--npcs`). Validado com `--desativar-ia` contra as 9 cidades reais (ver R04): 0 NPC sem casa, 0 id duplicado, 0 casa acima da capacidade padrão (5) com a faixa recalibrada — com a faixa inicial `[2, 4]` chegaram a existir 22 casas com 6 moradores (>5), sempre por CASAMENTO juntando dois membros adultos da mesma família a cônjuges externos, não por família sozinha (família sozinha nunca passa de 4) — dentro do esperado: é exatamente o gatilho de superlotação que `housing.py::processar_habitacao` já existe pra tratar (>2 adultos com cônjuge dispara nova obra), não uma falha de geração. Suíte: 164 passed. |
| R04 | 2026-09-14 | Mundo regerado do zero (`cartographer/reset_cartography.sh` + `populate.py --desativar-ia`) — **9 cidades** nesta semente (3 continentes × 3, não as 14 que a conta aritmética original de R01 estimava; contagem de cidade por continente é decisão de outro bloco, fora de escopo aqui). Achado que muda o Ação 2 do texto: `cidade_geo_domicilios_alvo_faixa_por_tamanho` grampeia nos DOIS lados, não só no piso — as 8 cidades de modelo "disco" (radial/grade/organica) grampearam pra CIMA (todo valor da faixa pede raio menor que o piso: pequeno 260m, médio 470m, grande 720m); a única cidade `linear` (Cormirmont) grampeou pra BAIXO, no teto (760m) — geometria "fita" precisa de MUITO mais raio pro mesmo tanto de lote. Nos dois casos o raio final não muda dentro da faixa atual de domicílios — a chave ficou matematicamente inerte pra baixar o total (só serve pra aumentar, se a faixa subir o bastante pra furar o piso disco ou descer o bastante pra caber sob o teto linear). Como o texto autoriza mexer só nessa chave e ela está inerte pra reduzir, a decisão registrada (não escondida) foi recalibrar `geracao_populacao.npcs_por_familia_faixa` (R03) de `[2,4]` pra `[2,3]` em vez dela — resultado: **23.788 NPCs, dentro de 22.000-28.000**. Tabela alvo-vs-medido (domicílios/lotes ALVO não ficam persistidos por cidade — sorteados em memória — reportados como grampeados/N-A; lotes REAIS e raio vêm de `database/cidades/_indice.json`, famílias/NPCs do banco): Vila Velha (radial, pequeno, raio=260m[piso], lotes=891, famílias=177, npcs=456) · Cidade dos Ventos (grade, médio, 470m[piso], 1691, 556, 1402) · Cidade dos Dragões (radial, médio, 470m[piso], 2505, 872, 2208) · Cormirmont (linear, médio, 760m[teto], 1114, 342, 865) · Elinford (radial, médio, 470m[piso], 2761, 931, 2316) · Lorverstead (radial, médio, 470m[piso], 2180, 741, 1862) · Rendormont (organica, médio, 470m[piso], 2395, 796, 1979) · Pelinfield (grade, grande, 720m[piso], 5327, 2699, 6773) · Quenorport (radial, grande, 720m[piso], 5243, 2381, 5927). Total: 9.495 famílias, média 2,505 NPCs/família (bate com o sorteio uniforme de `[2,3]`, média teórica 2,5). Verificação do efeito colateral de zoom (Ação 3): não é regressão — `GeradorCidade.raio_m = modelo.raio_m` (o raio REAL derivado, não nominal) e `zoom_min_por_camada(config, self.raio_m)` são chamados com esse valor tanto no gerador principal quanto em `expansao.py` (que lê `raio_m` de volta do GeoJSON persistido) — nenhum call site ficou com o raio nominal antigo. `prewarm_cache.py` rodou sem erro nos zooms baixos durante o reset. **Não consegui abrir o mapa no navegador nesta sessão** (mesma limitação já registrada em S03) — validado só por leitura de código + a regeração/prewarm não ter quebrado; inspeção visual de zoom numa cidade pequena fica pendente pro dono do projeto. Suíte: 164 passed (rodada depois de R01-R04 inteiros). **Achado adicional, depois de implementar Newton (Ação 5) e regerar o mundo de novo pra testar**: uma segunda regeração, mesma configuração, saiu com uma MISTURA de tamanho de cidade bem diferente da primeira (0 `grande` em vez de 2) — só isso derrubou o total pra 12.302 NPCs, bem abaixo de 22.000. Quantas cidades de cada porte o mundo funda é decisão do `[AI-CITY-STRATEGY]` (`cartographer/world/`), não sorteado por um `rng` local determinístico — é uma chamada de IA de verdade, fora do escopo de R01-R04 e fora do controle de `npcs_por_familia_faixa`. **A calibragem de `[2,3]` bate o alvo pra uma mistura de porte "típica" (a da primeira regeração, com `grande`), mas não garante 22k-28k pra toda regeração possível** — isso não é um bug de Bloco R, é uma dependência não documentada até agora entre "quantas cidades grandes a IA funda" e "o mundo cai dentro do alvo de população". Decisão pro dono do projeto: fora de escopo aqui, mas fica registrado — travar/pesar a proporção de porte de cidade (ex.: sempre fundar pelo menos 1 `grande` a cada N cidades) resolveria pela raiz; recalibrar `npcs_por_familia_faixa` não resolve (o problema é ausência de cidade grande, não família pequena demais). |
| W01 | 2026-09-14 | 8 dos 9 invariantes já existiam, escritos junto com a tarefa que os motivou (S01-S03/L01-L04/A01-A04/N02), só com nomes mais descritivos que a sugestão literal do texto — não é isso que faltava. O único que faltava de verdade: `test_raio_derivado_dos_domicilios` (`tests/test_cidades.py`), o invariante de R02. Escrevê-lo revelou que testar num raio ARBITRÁRIO (não um dos dois pontos calibrados) desvia até 93% numa cidade só pra `linear`/`grade` — motivou implementar a Ação 5 (Newton) primeiro, registrada na linha R02. Mesmo com Newton, uma cidade ISOLADA ainda pode passar de 15% (a correção reduz o pior caso, não zera o ruído de terreno/ocupação) — o teste mede a MEDIANA de 9 cidades-semente por modelo (mais que as 5 de `calibrar_densidade.py`, porque `linear` continuou beirando 15% até 9: com 5 sementes, um seed específico deu 15,7% de desvio, falso-negativo por variância de amostra pequena, não por bug). Suíte: 168 passed (4 casos novos, um por modelo). |
| W02 | | |
