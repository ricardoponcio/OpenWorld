...
> ([`12_PLANO_CIDADE_VIVA.md`](12_PLANO_CIDADE_VIVA.md) e
> [`13_PLANO_POPULACAO_E_ESCALA.md`](13_PLANO_POPULACAO_E_ESCALA.md)) foram executados quase por
...
Bloco C  (calibragem: invariante + anéis)
    │
    ▼
R01–R04 do 13_PLANO_POPULACAO_E_ESCALA.md   ← destravado por C01/C02, não reescrito aqui
    │
    ▼
...
o padrão que `AVANCAR_MINUTOS` já usa e que a `11_ARQUITETURA.md` já abençoa. Aí `acordar`
...
4. `mundo.acordar(npc)` passa a **mover** o NPC de balde, não só mudar o campo. Essa é a
   única forma de a porta continuar sendo porta ([armadilha 12](13_PLANO_POPULACAO_E_ESCALA.md)).
...
3. ⚠️ Mais anéis quer dizer mais ruas e mais quadras, logo **mais lotes** na mesma cidade.
   ...
   mesma [armadilha 8](13_PLANO_POPULACAO_E_ESCALA.md) do documento anterior, agora com um
   segundo motivo.
...
**Arquivos:** os do Bloco R do [`13_PLANO_POPULACAO_E_ESCALA.md`](13_PLANO_POPULACAO_E_ESCALA.md)
...
> 📗 **Existe um quarto documento:** [`15_PLANO_MUNDO_CRIVEL.md`](15_PLANO_MUNDO_CRIVEL.md).
> Ele não trata de geometria nem de velocidade — trata de **conteúdo**: o levantamento
> medido dos NPCs, da autossuficiência do mundo e da parametrização. Ele acrescenta
> quatro armadilhas (16 a 19), e duas delas tocam trabalho deste documento:
> a **16** (o benchmark só mede o que está dentro do `GameLoop` — 21% do custo real por
> dia simulado está fora) e a **17** (a agenda transformou taxa-por-minuto em
> taxa-por-avaliação, e o humor ficou ~3× mais lento). O Bloco C deste documento e o
> Bloco R do doc 2 continuam pendentes e continuam válidos — o doc 4 é insumo para
> eles, não substituto.
...
| H06 | 2026-09-14 | `bench_avanco.py` já ganhou `--por-hora`/`pico_decisoes_tick` no commit de H06 (parcial). 1 dia (25.000 NPCs, acumulado H01-H05): **62,6s (43,5 ms/tick)** — dentro da faixa que D22 revisou (9-12 ms/tick de piso + manada). **2 dias: 302,4s (105,0 ms/tick média) — NÃO é 2×63,4s.** Achado ao investigar a não-linearidade: instrumentei `npcs_em_consequencia`/`dinheiro_total_pc` diretamente (sem passar por bench_avanco) — o invariante de baldes continua batendo (nenhum vazamento na agenda: `baldes_total == vivos` nos dias 1 e 2, `n_baldes` estável em ~235-239) e `npcs_em_consequencia` fica em **0** nos dois dias (não é fome). O que muda: `dinheiro_medio` cai de 373,3 (dia 1) para 146,9 (dia 2) — a população sintética de `bench_tick.py` (`_construir_npcs`) **não recebe emprego** (nasce com `local_trabalho_id=""`), então nunca ganha salário e só gasta comendo; à medida que o saldo médio cai, `NPCActionManager.minutos_seguros_para_pular_comer` (H04) corretamente ENCOLHE a janela seguro de pular — o NPC fica mais pobre, o bloco de COMER fica menor, a agenda reavalia mais vezes, e isso se concentra exatamente nas horas de refeição (6/7/19-21), piorando dia após dia. **Não é um vazamento nem um bug de H01-H05** — é H04 reagindo corretamente a uma população sintética que a própria ferramenta de benchmark nunca empregou (pré-existente em `bench_tick.py`, não introduzido nesta sessão). Tentativas de medir 7 dias completos (900s e depois 1800s de orçamento, sem carga concorrente) não terminaram a tempo — a esta altura, isso deixou de ser "quanto tempo o benchmark leva" e virou uma pergunta sobre a ECONOMIA sintética, que é exatamente o que `docs/15_PLANO_MUNDO_CRIVEL.md` (achado externamente durante esta sessão, ver mensagem final) mede e resolve de verdade num mundo real (não sintético): a riqueza mediana do mundo real cai monotonicamente e a projeção chega a zero por volta do dia 35 — o mesmo fenômeno, medido de forma muito mais completa. D22 (7 dias em ~2 min) fica **não confirmado para uma corrida contínua de 7 dias**; confirmado e validado para 1-2 dias. Recomendação: remedir depois que o Bloco N do doc 4 (a economia fecha) estiver de pé, e considerar dar emprego à população sintética de `bench_tick.py` antes de rodar `--dias 7` de novo. |
...
| C03 | | |
| Y01 | 2026-09-14 | Os 6 testes pedidos foram escritos junto de cada tarefa, não depois, com nomes/arquivos ligeiramente diferentes do sugerido (mesma cobertura): `test_baldes_somam_a_populacao` (H01), `test_jitter_e_deterministico`/`test_jitter_nas_fronteiras_desincroniza_uma_populacao_homogenea` (H03), `test_comer_em_lote_da_o_mesmo_resultado_que_minuto_a_minuto` + equivalentes de CUIDAR_PROLE/CONSTRUIR (H04, no lugar de um único `test_refeicao_dura_o_mesmo` — a duração real de COMER é dirigida por fome, não por um número fixo de parcelas, então a equivalência bloco-vs-minuto é o invariante que de fato importa), `test_poda_nunca_descarta_familia` (H05), e `test_afetar_npc_muda_saude_humor_e_acorda`/`test_destruir_local_desativa_e_acorda_moradores_trabalhadores_e_presentes`/`test_reatribuir_npc_muda_trabalho_e_casa_reindexando_e_acorda` (F03, `test_acao_do_mestre_acorda_npc` virou três testes, um por ação). `test_lotes_batem_com_o_perimetro` NÃO foi duplicado em pytest — a auto-consistência de C01 já vive em `audit_cidades.py` (ferramenta de linha de comando, mesmo padrão de L04/S01), rodada a cada mudança de geometria; um segundo teste em `tests/test_cidades.py` testaria a mesma conta duas vezes. |
| Y02 | 2026-09-14 | `bench_tick.py --com-banco`: roda o MESMO cenário sintético duas vezes — uma com `BancoFalso` (CPU) e outra com um `DatabaseManager` de verdade num SQLite `tempfile` (WAL/synchronous=NORMAL, o próprio `DatabaseManager` já configura) — e imprime as duas colunas lado a lado. NPCs/locais são gravados ANTES de medir (`salvar_completo`/`salvar_em_lote`), senão o `UPDATE` de `salvar_muitos` (N02) não bateria em nenhuma linha e mediria o custo errado. Medido (25.000 NPCs/60.000 locais/40 cidades, 5 ticks): CPU 247,5 ms/tick, CPU+disco 265,4 ms/tick — delta de **17,9 ms (~7%)**, confirmando que N02 (UPDATE estreito só das colunas quentes) já deixou o custo de disco pequeno frente ao de CPU. |
