...
| A02 | 2026-09-12 | **PARADA — decisão pendente do dono do projeto, ver mensagem de fechamento da sessão.** Antes de escrever `proximo_instante_decisao`, percebi que "pular tempo" só é um pulo de verdade se os efeitos por minuto de uma ação EM ANDAMENTO (dormir ganha `energia_ganho` por tick em `actions.py`, trabalhar perde energia e paga salário por tick, comer recupera fome por tick, cuidar da prole consome energia por tick) também virarem função de `minutos` — do jeito que A01 fez pra metabolismo. Sem isso, "pular 4 horas" continuaria chamando `executar_acao` 240 vezes (só a decisão seria pulada, não o custo). Essa generalização não está detalhada tarefa por tarefa neste plano (A01 fala só de metabolismo/gravidez) e mexe na economia (salário, comida, sopão) — risco maior do que o texto de A02 sozinho sugere. Parei aqui em vez de decidir sozinho o alcance dessa mudança. |
| A03 | | |
| A04 | | |
| A05 | | |
| A06 | | |
