...
| R01 | | **PARADA — ver mensagem de fechamento da sessão.** R02 (calibração de densidade por medição real) fica invalidada se `cidade_geo_quadra_largura_alvo_m` (S01) mudar depois — e L04 já deixou essa mudança como decisão pendente. Calibrar R02 agora e descobrir depois que o dono do projeto quer outro `largura_alvo` significaria refazer a medição. Parei antes de R01/R02 por essa dependência, não por falta do que fazer. |
| R02 | | |
| R03 | | |
| R04 | | |
| W01 | | |
