...
| R02 | 2026-09-14 | `builder/fix/calibrar_densidade.py` (novo): gera a mesma cidade-semente em dois raios (300m/900m), 5 sementes por ponto, mediana, ajusta `lotes = k·raio^e` por modelo. Medido: `radial` k=0,014157 e=1,9654; `grade` k=0,009881 e=1,9881; `organica` k=0,019949 e=1,8882; `linear` k=0,048614 e=1,5064 — os três "disco" (e≈2) e o "fita" (e≈1,5) bateram com a expectativa qualitativa do plano. Colado em `cidade_geo_densidade_lote_por_modelo`. Newton correction (Ação 5 do texto, recalcular raio uma vez se o lote real desviar >15% do alvo) **não implementada** — o mundo medido em R04 nunca chegou a testar essa branch porque toda cidade já nasce grampeada num extremo da faixa de sanidade (ver R04), então o "alvo" nunca é o que decide o raio final; fica para quando/se a faixa de domicílios for recalibrada pra caber dentro do piso/teto. |
